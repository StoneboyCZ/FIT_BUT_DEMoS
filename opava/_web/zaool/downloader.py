...
if not len(os.listdir(dn)) == numberOfEntries: # not all HTMLs are downloaded
    ...
    for n in range(1,numberOfEntries+1,1):
        ...
        if os.path.isfile(fn): # skip 
            ...
            print(fn)
            if skip:
                url = 'http://digi.archives.cz/da/PaginatorResult.action?rowTxt='+str(skipIndex+1)+'&_sourcePage=tVZwK1Z3HOxPw0latj67Sp263nk16sdF9kzdDs5lA4TKb8_htHeO1BS-ffH4MkC49qSSx5aR0Nx9L3gK_Q_LkFVlKIsthpJAjzS6NdZ81MM%3D&__fp=HHR4eeeN9MX_IudliuvpVszfTgVdDAe_NbYwja2LX74e-x5sgFu5Mkhs7HO4Wmfm'
                r = s.get(url,cookies=cookies)  
                content = r.text
                skip = False
